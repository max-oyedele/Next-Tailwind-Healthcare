  ...
  return (
    <div className="px-6 md:px-36 md:grid md:grid-cols-2 md:gap-x-16 mt-10">
      ...
      <div className="flex flex-col mt-6">
        <span className="text-xl text-gray-900 font-bold">Bold density DEXA scan</span>
        <span className="text-md text-gray-800 mt-2">Protect your bones</span>
        <div className="mt-4">
          <StarRating />
        </div>
        <hr className="border-gray-300 my-4" />
        <span className="text-sm text-gray-700">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam morbiaenean sed uma massa quam. Sed consequat vitae, fusce diam sapien, phasellus. Turpis ultrices lectus et consequat. Scelerisque erat feugiat tortor imperdiet leo in. Ut mauris, orci, imperdiet est aliquam</span>
        <hr className="border-gray-300 my-4" />
...
const ProsConsContainer = () => {
  return (
    <div className="px-6 md:px-36 my-16">
      <span className="text-2xl text-gray-900 font-bold">What are the pros and cons of it</span>
      <div className="sm:grid sm:grid-cols-2 sm:gap-x-4">
        <div className="bg-gray-100 flex flex-col mt-6 p-4">
          <span className="text-md text-gray-900 font-bold">Pros</span>
          <div className="flex mt-4"><BiCheck className="text-gray-700" /><span className="text-sm text-gray-700 ml-1">Eliminates or minimizes wrinkles</span></div>
          <div className="flex mt-4"><BiCheck className="text-gray-700" /><span className="text-sm text-gray-700 ml-1">Results within 2-7days, with nearly 100% efficacy</span></div>
          <div className="flex mt-4"><BiCheck className="text-gray-700" /><span className="text-sm text-gray-700 ml-1">Convenient, with no downtime</span></div>
          <div className="flex mt-4"><BiCheck className="text-gray-700" /><span className="text-sm text-gray-700 ml-1">FDA-approved, with minimal health risks</span></div>
        </div>
        <div className="bg-gray-100 flex flex-col mt-6 p-4">
          <span className="text-md text-gray-900 font-bold">Cons</span>
          <div className="flex mt-4"><HiMinusSm className="text-gray-700" /><span className="text-sm text-gray-700 ml-1">Eliminates or minimizes wrinkles</span></div>
          <div className="flex mt-4"><HiMinusSm className="text-gray-700" /><span className="text-sm text-gray-700 ml-1">Results within 2-7days, with nearly 100% efficacy</span></div>
          <div className="flex mt-4"><HiMinusSm className="text-gray-700" /><span className="text-sm text-gray-700 ml-1">Convenient, with no downtime</span></div>
          <div className="flex mt-4"><HiMinusSm className="text-gray-700" /><span className="text-sm text-gray-700 ml-1">FDA-approved, with minimal health risks</span></div>
        </div>
      </div>
    </div>
  )
...
const StepContainer = () => {
  ...
  return (
    <div className="px-6 md:px-36 my-16">
      <span className="text-2xl text-gray-900 font-bold">Our Personalized Approach</span>
      <div className="sm:grid sm:grid-cols-3 sm:gap-x-4">
        {
          steps.map((step, index) => (
            <div key={index} className="bg-gray-100 flex flex-col p-4 rounded-md mt-6">
              <span className="text-sm text-gray-900 font-bold uppercase underline" style={{ textDecorationColor: '#f2cc8f', textUnderlineOffset: 8 }}>{step.step}&nbsp;&nbsp;&nbsp;</span>
              <div className="bg-white flex justify-center items-center rounded-md h-36 mt-6">
                <img src={step.img} height="100%" alt="stepImg" />
              </div>
              <span className="text-md text-gray-900 font-bold mt-6">{capitalize(step.title)}</span>
              <span className="text-sm text-gray-700 mt-2">{capitalize(step.desc)}</span>
            </div>
          ))
        }
      </div>
  ...
  return (
    <div className="px-6 md:px-36 my-16">
      <div className="flex flex-col items-center">
        <span className="text-2xl text-gray-900 font-bold">Other treatment options</span>
        <span className="text-md text-gray-800 mt-6">Find what's right for you.</span>
      </div>
        ...
        <div className="w-full md:w-3/4 sm:grid sm:grid-cols-2 sm:gap-x-6">
          {
            data.map((item, index) => (
              <div key={index} className="bg-gray-100 px-4 py-6 flex flex-col rounded-md mt-6">
                ...
                <span className="text-sm text-gray-900 mt-10">{item.tag}</span>
              </div>
            ))
          }
        </div>
  ...
  return (
    <div className="px-6 md:px-44 sm:grid sm:grid-cols-2 sm:gap-x-6">
      ...
      <div className="md:px-10 flex flex-col justify-center">
        <span className="text-xl text-gray-900 font-bold mt-10">The Because Health difference</span>
        <span className="text-sm text-gray-800 font-bold mt-6">Premium prescriptions on-demand</span>
        <span className="text-sm text-gray-700 mt-4">Get access to exclusive treatments targeted to your needs and your body, all for less.</span>
        <span className="text-sm text-gray-800 font-bold mt-6">Specialist access without the wait</span>
        <span className="text-sm text-gray-700 mt-4">Don't wait to see a specialist. Connect with top physicians on your time, from anywhere.</span>
        <span className="text-sm text-gray-800 font-bold mt-6">Scientifically-proven results</span>
        <span className="text-sm text-gray-700 mt-4">Research-based treatments and targeted care for faster, more consistent results every time.</span>
        <span className="text-sm text-gray-800 font-bold mt-6">Targeted testing and diagnoses</span>
        <span className="text-sm text-gray-700 mt-4">Individualized diagnostics ensure you're getting the right treatment for your unique needs.</span>
      </div>
    </div>
  )
}
...
const ApiReviewContainer = () => {
  return (
    <div className="flex justify-center items-center h-72 sm:h-96 my-16">
      <span className="text-2xl text-gray-900 font-bold">API Reviews</span>
    </div>
  )
}
