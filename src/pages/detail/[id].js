  ...
  return (
    <div className="px-6 md:px-36 md:grid md:grid-cols-2 md:gap-x-10 mt-6">
      ...
      <div className="mt-10 flex flex-col">
        <span className="text-lg text-gray-900 font-bold">Awaken Nasal Spray</span>
        <span className="text-sm text-gray-700 mt-2">For increased sexual desire</span>
        <div className="mt-4">
          <StarRating />
        </div>
        ...
        <span className="text-md text-toneblue-light cursor-pointer mt-4" onClick={() => props.setPlanLayer(true)}>See plan options {'>'}</span>
        ...
        <div className="flex items-center mt-6">
          ...
          <span className="text-sm text-gray-900 ml-2">Clinically proven treatment for increased sexual desire</span>
        </div>
        <div className="flex items-center mt-6">
          <BsCircleFill className="w-2 text-gray-900" />
          <span className="text-sm text-gray-900 ml-2">Puts control back in your hands, when you want it</span>
        </div>
        <div className="flex items-center mt-6">
          <BsCircleFill className="w-2 text-gray-900" />
          <span className="text-sm text-gray-900 ml-2">Take as needed without any alcohol restrictions</span>
        </div>
        <div className="flex items-center mt-6">
          <BsCircleFill className="w-2 text-gray-900" />
        ...
        <span className="text-xl text-gray-900 font-bold mt-10">A solution you can rely on</span>
        <span className="text-md text-gray-900 font-bold mt-6">How it works</span>
        <span className="text-sm text-gray-800 mt-4">This powerful spray helps restore the chemical balance that leads to sexual excitement -- and can keep working for up to 18 hours. An easy-to-use, on-demand treatment, this spray sends a chemical signal to your bracin that triggers sexual desire. It works as-needed to amp up your sex drive quickly and easily, with no side effects and no lag.</span>
        <span className="text-md text-gray-900 font-bold mt-6">How to use</span>
        <span className="text-sm text-gray-800 mt-4">Spray one puff of Awaken Nasal Spray in each nostril 30-60 minutes prior to sexual activity.</span>
        ...
        <span className="text-md text-toneblue-light cursor-pointer mt-4">Important safety information {'>'}</span>
      </div>
    </div>
  )
}
...
const TextContainer = () => {
  return (
    <div className="flex flex-col items-center my-16" style={{ backgroundColor: '#e4eff3' }}>
      ...
      <span className="text-md text-toneblue-light cursor-pointer my-6">See plan options {'>'}</span>
    </div>
  )
}
      ...
      <div className="absolute inset-0 flex justify-end">
        <div className="bg-gray-300 w-full sm:w-2/3 md:w-1/3 p-10">
          ...
          <div className="h-full flex flex-col justify-center">
            <span className="text-2xl text-gray-900 font-bold">Plan Options</span>
            <span className="text-sm text-gray-700 mt-10">We offer two subscription plans. Choose your preferred option during checkout.</span>
            {
              data.map((item, index) => (
                <div key={index} className="bg-white p-2 flex flex-col mt-10">
                  <span className="text-md text-gray-900 font-bold">{item.title}</span>
                  <span className="text-md text-gray-900 mt-2">${item.price} / month</span>
                  <span className="text-xs text-gray-700 mt-4">{item.desc}</span>
                </div>
              ))
            }
            <div className="flex flex-col items-center mt-10">
              <span className="text-sm text-gray-700">Free shipping on all orders over $75.</span>
